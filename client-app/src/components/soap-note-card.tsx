...
export const SoapNoteCard = ({
  ...
  const handleDelete = async () => {
    ...
    if (!confirm("Are you sure you want to delete this SOAP note? This action cannot be undone.")) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(result.message || "Failed to delete SOAP note");
      }
    } catch (error) {
    ...
    try {
      ...
      const formattedNote = {
        ...
        subjective: {
          ...
          purpose_of_appointment: filterEmpty(editedNote.subjective?.purpose_of_appointment),
          others: editedNote.subjective?.others || "",
        },
        objective: {
          physical_exam_report: filterEmpty(editedNote.objective?.physical_exam_report),
      ...
      if (response.ok) {
        ...
        throw new Error(result.message || "Failed to update SOAP note");
      }
    } catch (error) {
      ...
      <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              {isEditing ? (
                <Textarea
                  value={
                    ((editedNote.subjective?.purpose_of_appointment ?? []) as string[])
                      .map(p => p.replace(/_/g, " "))
                      .join("\n")
                  }
                  readOnly
        ...
        {(soapNote.objective || isEditing) && (
          <div className="space-y-3">
            ...
            <div>
              ...
              {isEditing ? (
                <Textarea
                  value={editedNote.objective?.physical_exam_report?.join("\n") || ""}
                  onChange={(e) =>
                    setEditedNote(prev => ({
                      ...prev,
                      objective: {
            ...
            {(soapNote.objective?.labs && Object.keys(soapNote.objective.labs).length > 0) || isEditing ? (
              <div>
                ...
                />
                  ...
                  <div className="p-3 rounded-lg">
                    {soapNote.objective?.labs && Object.entries(soapNote.objective.labs).map(([key, value], index) => (
        ...
        {(soapNote.plan || isEditing) && (
          <div className="space-y-3">
            ...
            {!isEditing && soapNote.plan?.prescription && soapNote.plan.prescription.length > 0 && (
              <div>
                ...
                soapNote.plan?.test_requests ? (
                  <div className="flex flex-wrap gap-2">
                    {Array.isArray(soapNote.plan.test_requests) ? (
                      ...
                      Object.values(soapNote.plan.test_requests).map((test, index) => (
                        <Badge key={index} variant="secondary">
                          {test}
                        </Badge>
                      ))
                ...
                <Textarea
                  value={
                    Array.isArray(editedNote.plan?.recommendation)
                      ...
                      ? Object.values(editedNote.plan.recommendation || {}).join("\n")
                      : ""
                  }
                  onChange={(e) => {
                    const values = e.target.value.split("\n").filter(line => line.trim());
                ...
                soapNote.plan?.recommendation ? (
                  <div className="flex flex-wrap gap-2">
                    {Array.isArray(soapNote.plan.recommendation) ? (
                      ...
                      Object.values(soapNote.plan.recommendation).map((rec, index) => (
                        <Badge key={index} variant="secondary">
                          {rec}
                        </Badge>
                      ))

...
export type AppointmentProviderSchema = z.infer<
  typeof appointmentProvidersSchema
>
...
const nestedFollowUpAppointmentsSchema = z
  .object({
...
const nestedFollowUpAppointmentSchema = z
  .object({
...
const appointmentRegisterSchema = z
  .object({
    patient_id: z.object({
      id: z.uuid(),
  ...
  .refine(
    (data) =>
      !data.has_insurance || Boolean(data.patient_id.insurance_provider_id),
    {
  ...
  .superRefine((data, ctx) => {
    ...
    if (hasOthers && !data.other_purpose) {
      ctx.addIssue({
        ...
        code: 'custom',
        message:
          "You must provide 'other_purpose' when 'OTHERS' is selected in purposes",
      })
    }
    ...
    if (
      ...
      typeof data.is_follow_up_required !== 'boolean'
    ) {
      ctx.addIssue({
        path: ['is_follow_up_required'],
        code: 'custom',
        message:
          "'is_follow_up_required' is required when status is 'COMPLETED'",
      })
    }
    ...
    if (data.status !== 'COMPLETED' && 'is_follow_up_required' in data) {
      ctx.addIssue({
        ...
        code: 'custom',
        message:
          "'is_follow_up_required' should only be provided when status is 'COMPLETED'",
      })
    }
  })

export type AppointmentRegisterSchema = z.infer<
  typeof appointmentRegisterSchema
>
